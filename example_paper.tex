...
\icmlauthor{Arwin Sadaghiani}{equal,yyy}
...
For the DeepONet model, we created a dataset containing 100K random one-dimensional, target interpolation functions $f(\tau)$ on the interval $[0, 1]$, defined on a fine grid of 128 points.
We sampled them from Gaussian Processes with an RBF kernel and mean $0$. The kernel is defined as:
\begin{equation*}
    k(\tau, \tau') = \sigma^2 \cdot \exp\left(-\frac{||\tau - \tau'||^2}{2 \cdot l^2}\right)
\end{equation*}
It has two hyperparameters, the scale $l$, which controls the scale at which variations take place and the variance $\sigma^2$.
...
Depending on whether or not we are creating a train or test dataset, the alpha and beta values for the beta distributions are randomly chosen from either $[1.0,2.0,5.0]$ or $[0.5,3.25,7.8]$ respectively. The variance for the kernel is drawn uniformly from the interval $[0.5, 2]$.
we then sample random observations grid points $\tau$ on our functions, which become the input for our model. We sample a random subset between “min=50” and “max=90” points from the 128 grid points and pad the rest with $0$'s. We also create a binary mask for each observation grid, which tracks which indices are observed versus unobserved, that gets fed into our model along with the points.
Half of our observation points are sampled regular and the other half irregular (in time). Since data is noisy in real-world applications we add Gaussian noise to the observations with a standard deviation drawn from $\mathcal{N}(0, 0.1)$ and mean $0$ after first normalizing our data.
We normalize the observations to $[0, 1]$ and then perform z-scoring to normalize the function values, which then gets us our normalized values like this:
\begin{align*}
    \hat{f}(\tau) &= \frac{f(\tau) - \mu}{\sigma}\\
    \hat{\tau} &= \frac{\tau - \tau^{min}}{\tau^{max} - \tau^{min}}
\end{align*}
With $\mu$ and $\sigma$ being the mean and standard deviation of the function values and $\tau^{min}$ and $\tau^{max}$ being the minimum and maximum of the observation points.\\
For the FIM model we created a dataset containing 100K random one-dimensional, target functions $f(\tau)$ on the interval $[0, 1]$, defined on a fine grid of 640 points.
We sampled them from Gaussian Processes with mean $0$ and different kernels, namely a periodic kernel $k_{p}$ (30\%), a locally periodic kernel $k_{lp}$ (30\%), a linear plus periodic kernel $k_{lpp}$ (20\%)  and a linear times periodic kernel $k_{ltp}$ (20\%).
The kernels are defined as:
\begin{align*}
    k_{p}(\tau, \tau') &= \sigma^2 \cdot \exp\left(-\frac{2\sin^2(\pi|\tau - \tau'|/p)}{l^2}\right) \\
    k_{lp}(\tau, \tau') &= k_{p}(\tau, \tau') \cdot \exp\left(-\frac{(\tau - \tau')^2}{2l^2}\right) \\
    k_{lpp}(\tau, \tau') &= k_{p}(\tau, \tau') + \sigma^2 \cdot (\tau \cdot \tau'^T) \\
    k_{ltp}(\tau, \tau') &= k_{p}(\tau, \tau') \cdot \sigma^2 \cdot (\tau \cdot \tau'^T)
\end{align*}
On top of the length $l$ and variance $\sigma^2$ the kernels now also have a periodicity parameter $p$. Both the length and periodicity are drawn from a beta distribution again. This time the values for the distribution are drawn from either $[1.0,2.0,5.0]$ or $[1,1,1]$.
The variance is drawn the same way as for the DeepONet dataset. We again sample random observations grid points $\tau$ on our functions, which become the input for our model. However since we want to predict the future embedding $k+1$ we split the observations into $5$ windows with $k=4$. Thus we only sample observations from the first $540$ points.
...
On thing to note is that before we perform the normalizations of each $128$-point window we first normalize the whole function. Inspired by \cite{fim-l} we save the local statistics of each window into a scales array $s_l$, with:
$s_l = [f(\tau)^{min}, f(\tau)^{max}, f(\tau)^{first}, f(\tau)^{last}, f(\tau)^{first} - f(\tau)^{last}, \tau^{first}, \tau^{last}, \tau^{first} - \tau^{last}]$. With $\tau$ being our sampled observations and $f$ being our noisy function values.
